    ...
    def train(self, model_pattern):
        ...
        for epoch in range(self.config.max_epoch):
            ...
            for idx, data in enumerate(train_data_loader):
                ...
                sub_heads_loss = loss(data['sub_heads'], pred_sub_heads, data['mask'].float())
                sub_tails_loss = loss(data['sub_tails'], pred_sub_tails, data['mask'].float())
                obj_heads_loss = loss(data['obj_heads'], pred_obj_heads, data['mask'].float())
                obj_tails_loss = loss(data['obj_tails'], pred_obj_tails, data['mask'].float())
                total_loss = (sub_heads_loss + sub_tails_loss) + (obj_heads_loss + obj_tails_loss)
                print('epoch: {}, step: {}, train loss: {:5.3f}'.format(epoch, idx, total_loss.item()))
                optimizer.zero_grad()
                total_loss.backward()
                optimizer.step()
                ...
                if global_step % self.config.period == 0:
                    ...
                    self.logging("epoch: {:3d}, step: {:4d}, speed: {}min/bs, train loss: {:5.3f}".
                                 format(epoch, global_step, elapsed//60//self.config.period, cur_loss))
                    loss_sum = 0
                    start_time = time.time()
            ...
            if (epoch + 1) % self.config.test_epoch == 0:
                ...
                model.train()
                self.logging('epoch {:3d}, eval time: {}min, f1: {:4.2f}, precision: {:4.2f}, recall: {:4.2f}'.
                             format(epoch, (time.time() - eval_start_time)//60, f1_score, precision, recall))
